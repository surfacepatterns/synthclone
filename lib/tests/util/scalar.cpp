...
namespace {
    ...
    test_arithmetic_comparisons()
    {
        ...
        BOOST_CHECK(proxy_copy_2 == static_cast<S>(0));
        proxy_copy_2 = proxy;

        BOOST_CHECK(proxy > value_dec);
        BOOST_CHECK(proxy >= value_dec);
        BOOST_CHECK(! (proxy == value_dec));
        BOOST_CHECK(proxy != value_dec);
        BOOST_CHECK(! (proxy <= value_dec));
        ...
        BOOST_CHECK(! (proxy > proxy_copy_2));
        BOOST_CHECK(proxy >= proxy_copy_2);
        BOOST_CHECK(proxy == proxy_copy_2);
        BOOST_CHECK(! (proxy != proxy_copy_2));
        BOOST_CHECK(proxy <= proxy_copy_2);
    ...
    test_explicit_conversion()
    {
        ...
        BOOST_CHECK((n == static_cast<T>(value)));
    }
    ...
    test_formatting()
    {
        ...
        BOOST_CHECK(value_string == proxy_string);
    }
    ...
    test_hashing()
    {
        ...
        BOOST_CHECK(value_hash == proxy_hash);
    }
    ...
    test_ostream_output()
    {
        ...
        BOOST_CHECK(value_stream.str() == proxy_stream.str());
    }
...
BOOST_AUTO_TEST_CASE(enum_comparisons)
{
    ...
    BOOST_CHECK((one_proxy > zero_proxy));
    BOOST_CHECK((one_proxy >= zero_proxy));
    BOOST_CHECK((! (one_proxy == zero_proxy)));
    BOOST_CHECK((one_proxy != zero_proxy));
    BOOST_CHECK((! (one_proxy <= zero_proxy)));
    ...
    BOOST_CHECK((! (zero_proxy > one_proxy)));
    BOOST_CHECK((! (zero_proxy >= one_proxy)));
    BOOST_CHECK((! (zero_proxy == one_proxy)));
    BOOST_CHECK((zero_proxy != one_proxy));
    BOOST_CHECK((zero_proxy <= one_proxy));
    ...
    BOOST_CHECK((one_proxy > zero_value));
    BOOST_CHECK((one_proxy >= zero_value));
    BOOST_CHECK((! (one_proxy == zero_value)));
    BOOST_CHECK((one_proxy != zero_value));
    BOOST_CHECK((! (one_proxy <= zero_value)));
    ...
    BOOST_CHECK((! (zero_proxy > one_value)));
    BOOST_CHECK((! (zero_proxy >= one_value)));
    BOOST_CHECK((! (zero_proxy == one_value)));
    BOOST_CHECK((zero_proxy != one_value));
    BOOST_CHECK((zero_proxy <= one_value));
    ...
    BOOST_CHECK((zero_proxy == zero_proxy_copy));
}
